...
const indianNames = [
  ...
  "Sneha Gupta", "Arjun Mehta", "Kavita Nair", "Rohan Joshi", "Meera Iyer",
...
async function seedCallLogs(prisma: PrismaClient, businessId: string) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const callLog = {
      ...
      recipientPhone: business.twilioPhoneNumber || "+91987654321",
...
function generateSummary(): string {
  const summaries = [
    ...
    "Family consultation booking. Multiple family members.",
...
function generateStructuredNotes(): string[] {
  const allNotes = [
    "Customer expressed interest in premium services",
    "Urgent assistance required",
    "Flexible with appointment timing",
    "Requested pricing information",
    "Previous customer with good history",
    "New customer inquiry",
    "Needs detailed consultation",
    "Interested in membership plans",
    "Requested callback for more details",
...
function generateBookingNotes(serviceName: string): string {
  const notes = [
    ...
    `First-time customer for ${serviceName.toLowerCase()}.`,
    `Returning customer for ${serviceName.toLowerCase()}.`,
    `Urgent ${serviceName.toLowerCase()} requested.`,
    `Flexible timing requested for ${serviceName.toLowerCase()}.`,
    `${serviceName.toLowerCase()} for multiple family members.`,
    `Premium ${serviceName.toLowerCase()} package inquiry.`,
    `Standard ${serviceName.toLowerCase()} service.`,
    `Emergency ${serviceName.toLowerCase()} needed.`,
  ];
